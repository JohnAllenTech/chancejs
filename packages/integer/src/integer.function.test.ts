...
describe('integer function', () => {
  ...
  it('can take a zero min and obey it', () => {
    times(1000, () => {
      expect(integer({ min: 0 })).toBeGreaterThan(-1)
    })
  })

  it('can take a negative min and obey it', () => {
    times(1000, () => {
      expect(integer({ min: -25 })).toBeGreaterThan(-26)
    })
  })

  it('can take a negative min and max and obey both', () => {
    times(1000, () => {
      const n = integer({ min: -25, max: -2 })
  ...
  it('can take a min with absolute value less than max and return in range above', () => {
    let count = 0
    times(1000, () => {
      const n = integer({ min: -1, max: 1_000_000 })
  ...
  it('throws an error when min > max', () => {
    expect(() => integer({ min: 1000, max: 500 })).toThrow(RangeError)
  })
})
